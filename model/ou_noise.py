...
class OrnsteinUhlenbeckActionNoise:
    ...
    def __init__(self, mu, theta=0.15, max_sigma=0.3, min_sigma=0.1, dt=1e-2, x0=None, decay_period=100000, epsilon_start=1, epsilon_decay=0.002, epsilon_min=0.001):
    ...
    def __call__(self, t=0):
        ...
        self.x_prev = x
            # noise = self.theta * (self.mu - self.x_prev) * self.dt + self.sigma * np.sqrt(
            #     self.dt) * np.random.normal(size=self.mu.shape)
            # x = self.x_prev + noise * self.epsilon
            # self.x_prev = x
            # print(' OU Noise: ', x)
        return x
        # else:
            # print(' OU Noise: ', np.zeros_like(self.mu))
            # return np.zeros_like(self.mu)
            # x = np.zeros_like(self.mu)
        # x = self.x_prev + self.theta * (self.mu - self.x_prev) * self.dt + self.sigma * np.sqrt(
        #         self.dt) * np.random.normal(size=self.mu.shape)
        # self.x_prev = x
        # print(' OU Noise: ', x)
